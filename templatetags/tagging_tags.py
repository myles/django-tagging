...
class TagsForObjectNode(Node):
	...
	def render(self, context):
		context[self.context_var] = Tag.objects.get_for_object(self.obj.resolve(context))
		return ''
...
class TaggedObjectsNode(Node):
	...
	def render(self, context):
		...
		context[self.context_var] = TaggedItem.objects.get_by_model(model, self.tag.resolve(context))
		return ''
